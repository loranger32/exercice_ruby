...
class QueenAttackTest < Minitest::Test
  ...
  def test_must_have_two_queens_to_say_if_attack_is_possible
    #skip
    assert_raises StandardError do
      Queens.new(white: [4, 7]).attack?
    end
  end
  ...
  def test_can_attack_on_fifth_diagonal
    #skip
    queens = Queens.new(white: [7, 3], black: [3, 7])
    assert queens.attack?
  end  

  def test_first_wrong_diagonal
    #skip
    queens = Queens.new(white: [2, 2], black: [1, 7])
    refute queens.attack?
  end

  def test_two_queens_cannot_have_same_postion
    #skip
    assert_raises(ArgumentError) { Queens.new(white: [2, 2], black: [2, 2]) }
  end
